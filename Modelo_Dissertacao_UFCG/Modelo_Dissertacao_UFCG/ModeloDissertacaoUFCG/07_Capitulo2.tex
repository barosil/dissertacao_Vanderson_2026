...
\todo{Termos em inglês devem estar enfatizados com \emph{single dish}}
...
\todo{Utilize as referências BINGO I e BINGO II no arxiv para descrever o instrumento. É um radiotelescópio cross-dragone de trânsito com feixes mútiplos (multibeam), eu vou selecionar as imagens, e coloque uma tabela com os parâmetros (igual a do artigo)}
...
digital (ADC) no qual o sinal analógico é convertido, canalizado em frequência e armazenado na forma de dados digitais \cite{ramalho2023impactos}. \todo{o filtro passa banda do BINGO não é sintonizável, e não tem conversor de frequência.}
...
%---------------------------------------------------------------------------------------------------------------------------------------------------------
\todo{Eu acho que seria melhor inverter a ordem e colocar o conteúdo desta seção antes da parte sobre tipos de observação.}

\todo{Aqui eu acho que precisa ter as definições fundamentais, de padrão de radiação, ganho, diretividade, âmgulo sólido, FWHM, temperatura de brilho, temperatura de antena. O Wilson Rohls é o melhor para isto, ou o Kraus.}

\todo{Me cobre uma figura 3D de padrão de radiação}
...
Radiotelescópio que recebe sinais muito fracos de suas radiofontes, até mesmos a radiação térmica do solo ( com temperatura próxima \(T \approx 300K\)) que são captadas pelos lóbulos secundários, acabam interferindo na relação sinal-ruido (S/R), assim como, a radiação recebida de fora da borda do refletor principal (spillover) também prejudica a recepção do sinal emitido pela radiofonte \cite{sasao2009_radioantennas}, como representa a figura \ref{fig:Lobulo principal/secundario}. \todo{Talvez a figura 3D que eu vou mandar seja melhor que estya, de uma forma ou de outra, esta figura está associada para uma expressão impoortante da eficiência da antena, como o produto de vários termos de perda de sinal, Wilson tem esta extressão e vale a pena colocar.}
...
\todo{isto depende do feixe, não tem um valor típico para uma frequencia.}
...
No regime \textit{Single Dish}, como no caso do experimento BINGO, o interesse observacional está na intensidade total do sinal recebido por cada feixe individual. Nesse contexto, o backend implementa um correlacionador de autocorrelação, no qual o sinal é correlacionado consigo mesmo para a obtenção do espectro de potência por canal de frequência. \todo{single dish pode fazer um monte de coisas, isto não determina o tipo de receptor. As antenas são sensíveis a polarização, assim, usualmente se recebe duas polarizações, no caso do BINGO, cada corneta produz 4 sinais, 1 sinal para cada polarização e 1 sinal de referência para cada polarização, o backend faz uma pseudo-correlação entre o sinal de cada polarização com sua referência, devolvendo dois sinais. A pseudo correlação é essencialmente a subtração de um sinal pelo outro, o que ajuda a estabilidade do sistema com relação a ruido 1/f}
\end{itemize}
...
No experimento BINGO, após a recepção e amplificação criogênica, o sinal analógico é convertido para uma frequência intermediária (IF) e submetido a um estágio de filtragem passa-banda analógica. Esse filtro define a faixa operacional do experimento, tipicamente entre aproximadamente 980–1260 MHz, correspondente ao intervalo de redshift alvo para o mapeamento da linha de 21 cm do hidrogênio neutro\cite{Wuensche2022} \todo{sem criogenia}
. Além de selecionar a banda científica, essa etapa atua como filtro anti-aliasing, prevenindo a superposição espectral decorrente da amostragem antes da digitalização \cite{MarcelobjApostilaPDS}. \todo{isto não é verdade, remova. Quem faz isto é um banco de filtros polifásico, implementado digitalmente.}
...
\todo{Nesta seção você pode gerar um mapa para cada um destes ruídos usando o pacote pysm3. A documentação dele é camarada, você pode gerar mapas healpix com N=1024, centrado em 1 GHz e garantir que a escala deles seja comparável.}
